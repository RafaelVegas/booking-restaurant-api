...
public class ReservationService implements ReservationServiceI{
	...
	private static final org.jboss.logging.Logger LOGGER = LoggerFactory.logger(ReservationService.class);
	...
	public String createReservation(CreateReservationRest createReservationRest) throws BookingException {		
		final Restaurant restauranteEntity = restaurantRepository.findById(createReservationRest.getId())
				.orElseThrow(()-> new NotFoundException("Error 404 ", "Restaurante no registrado con id: " + createReservationRest.getRestaurantId()));
		
		final Turn turnEntity = turnRepository.findById(createReservationRest.getTurnId())
				.orElseThrow(() -> new NotFoundException("Error 404 ", "Turno no encontrado con id: " + createReservationRest.getTurnId()));
		...
		try {
			...
			throw new InternalServerErrorException("Internal SERVER ERROR ", "La reservada no ha sido guardada correctamente");
		}
	...
	private Reservation findByReservationByIdEntity(Long id) throws BookingException{
		return reservationRepository.findById(id).orElseThrow(() -> new NotFoundException("Error 404 ", "Reserva no encontrada con id: " + id));
	}
	
	private String getLocator(final Restaurant restaurantEntity, final CreateReservationRest createReservationRest) throws BookingException{
